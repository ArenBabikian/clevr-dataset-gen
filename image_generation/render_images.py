...
parser.add_argument('--distinct_objects', default=False, action='store_true', 
                    help="Generate objects with distinct size+color combo")
# Output settings
...
def rotate(x, y, angle):
    ...
    y = math.cos(angle) * x + math.sin(angle) * y
    return x, y


def overlapping_object(pos, scene_struct, r):
    """
    Add a random object that visually overlaps with another object
    """

    dirs = scene_struct['directions']
    # dist is already normalized
    dist = args.min_dist + pos[2] + r
    sign = random.choice([-1, 1])
    boundary = max(pos[2], 1.1 * args.margin)
    margin = sign * (random.uniform(args.margin, boundary))
    dir_front = ((dirs['front'][0] * dist + dirs['left'][0] * margin),
                 (dirs['front'][1] * dist + dirs['left'][1] * margin))
    # rotation = math.pi * random.uniform(-1/6, 1/6)
    # dir_front = rotate(dir_front[0], dir_front[1], rotation)

    obj_pos = pos[0] + dir_front[0], pos[1] + dir_front[1]
    return obj_pos


def generate_random_legal_object(scene_struct, size_mapping, object_mapping, positions, shape_color_combos,
                                 color_name_to_rgba,
                                 material_mapping, object_combo, args):
    ...
    while True:
        ...
        if not args.distinct_objects or (size_name, color_name) not in object_combo:
            break
...
def generate_overlapping_object(scene_struct, size_mapping, object_mapping, positions, shape_color_combos,
                                color_name_to_rgba, material_mapping, objects, args):
    # Choose a random size
    size_name, r = random.choice(size_mapping)
    ...
    while True:
        ...
        index = random.choice(range(len(positions)))
        x, y = overlapping_object(positions[index], scene_struct, r)
        if not (-3 <= x <= 3) or not (-3 <= y <= 3):
            continue
    ...
    # Choose random color and shape
    obj_name, obj_name_out = random.choice(object_mapping)
    color_name, rgba = objects[index]['color'], color_name_to_rgba[objects[index]['color']]
